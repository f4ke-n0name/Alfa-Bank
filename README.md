...
## Лаборатория 3 — Экран авторизации

### Как запустить

1. Открыть `apexbank.xcodeproj` в Xcode.
2. Выбрать симулятор или устройство (iOS 16+).
3. `Cmd+R` — запуск.
4. Приложение сразу откроет экран авторизации.

### Данные для входа

| Поле     | Значение               |
|----------|------------------------|
| Email    | `user@apexbank.ru`     |
| Пароль   | `qwerty123`            |

### Что происходит при успешном входе

После корректного ввода credentials `AuthPresenter` вызывает `AuthRouter.openCardList(session:)`.
Роутер заменяет корень навигационного стека на `CardListStubViewController` — заглушку,
которая отображает подтверждение входа и session token. В консоль также выводится `[CardListStub] Вход выполнен.`

### Что происходит при неверных данных

- Inline-ошибки под полями (`showEmailError` / `showPasswordError`) с подсветкой рамки красным.
- Красный баннер под полями при неверной паре логин/пароль.
- Кнопка «Войти» блокируется на время проверки и показывает индикатор загрузки.
